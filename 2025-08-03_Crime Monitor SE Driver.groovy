...
def poll() {
    ...
    try {
        httpGet([uri: apiUrl, contentType: "application/json"]) { resp ->
            if (resp.status == 200) {
                state.timeoutCount = 0  // Reset on success
        ...
        if (e.message?.contains("Read timed out")) {
            state.timeoutCount = (state.timeoutCount ?: 0) + 1
            if (state.timeoutCount >= 3) {
                log.error "API timeout has happened ${state.timeoutCount} times in a row!"
            } else {
                log.warn "API timeout. (Consecutive: ${state.timeoutCount})"
            }
        } else {
