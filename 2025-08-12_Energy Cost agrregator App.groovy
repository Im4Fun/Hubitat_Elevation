...
def ensureChild() {
  ...
  if (!getChildDevice(dni)) {
    addChildDevice("gpt.tools", "Energy Summary Device", dni, [label: "Energy Summary", isComponent: true])
  }
  updateChild()
}
...
def updateChild() {
  try {
    ...
    child?.sendEvent(name: "todayEnergy", value: t.todayKwh.setScale(dk, BigDecimal.ROUND_HALF_UP) as String, unit: "kWh")
    child?.sendEvent(name: "monthEnergy", value: t.monthKwh.setScale(dk, BigDecimal.ROUND_HALF_UP) as String, unit: "kWh")
    child?.sendEvent(name: "price", value: t.price as String, unit: "${c}/kWh")
    child?.sendEvent(name: "todayCost", value: t.todayCost.setScale(dc, BigDecimal.ROUND_HALF_UP) as String, unit: c)
    child?.sendEvent(name: "monthCost", value: t.monthCost.setScale(dc, BigDecimal.ROUND_HALF_UP) as String, unit: c)
