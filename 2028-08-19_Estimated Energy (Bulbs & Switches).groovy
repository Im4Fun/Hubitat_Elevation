 ...
 *  - True TOU cost integration (ΔkWh * price_at_that_time)
 *
...
definition(
  ...
  namespace:   "calle",
  author:      "Carl Rådetorp",
  description: "Estimate kWh for bulbs/switches from standby & max W with TOU-accurate costs.",
  category:    "Green Living",
  singleInstance: false,
  iconUrl: "https://raw.githubusercontent.com/hubitat/community/master/icons/energy.png",
  iconX2Url: "https://raw.githubusercontent.com/hubitat/community/master/icons/energy@2x.png"
)
...
def initialize() {
  ...
  // Child handling
  if (settings?.createChild) { ensureChildPresent(); updateChild() }
}
...
def switchHandler(evt) {
  ...
  accrueAll()  // accrue using current lastPrice
  def dev = evt.device
...
def priceChanged(evt) {
  ...
  // Accrue everything up to NOW at the OLD price
  accrueAll()
  // Switch to the NEW price
  state.lastPrice = currentPricePerKwh() ?: 0G
  state.lastPriceTs = now()
  if (pushOnTick) updateChild()
}
...
/**
 * Accrue energy for all devices since their lastTs, and
...
def accrueAll() {
  ...
  // TOU Cost: all devices share the same price, so add once based on summed ΔkWh
  if (totalIncKWh > 0G) {
...
def totals() {
  BigDecimal tDayK   = sumMapBD(state.todayKwh)
  BigDecimal tMonthK = sumMapBD(state.monthKwh)
  BigDecimal priceNow = currentPricePerKwh() // current display only
  int dk = decK()
  int dc = decC()
  ...
  return [
    ...
    price    : priceNow?.setScale(3, BigDecimal.ROUND_HALF_UP),
...
def ensureChildPresent() {
  try {
    ...
    return child
  } catch (e) { log.error "Failed to ensure child: ${e}"; return null }
}

def deleteChildIfExists() {
  def child = getChildDevice(childDni())
  if (child) { try { deleteChildDevice(child.deviceNetworkId); log.info "Deleted child device." }
               catch (e) { log.error "Failed to delete child: ${e}" } }
}
...
Map childSummaryMap() {
  ...
  [
    ...
    lastUpdated: ts,
    powerNow:    currentTotalWatts().setScale(1, BigDecimal.ROUND_HALF_UP)
  ]
}
...
def updateChild(opts = null) {
  ...
  try {
    ...
    child.sendEvent(name:"energy",      value: m.monthEnergy as BigDecimal, unit:"kWh")  // EnergyMeter
    if (logEnable) log.debug "Child updated: ${m}"
  } catch (e) { log.warn "Failed to push to child: ${e}" }
}
...
def ensureState() {
  state.lastTs     = state.lastTs     ?: [:]  // per-device timestamps
  state.lastPower  = state.lastPower  ?: [:]  // per-device W
