/**
 * INNR AE264 / RB286C Zigbee Dimmable Bulb Driver for Hubitat Elevation
 *
 ...
 *  1.0.2 - Fix for color temperature
 *  1.2.0 - Added fingerprint for RB 286
 *  1.2.1 - Fix: add healthCheck() to satisfy platform and stop MissingMethodException
 *          Harden parse() to avoid any chance of divide-by-zero on mireds
 */

metadata {
    definition(name: "INNR AE264 / RB286C Dimmable Bulb", namespace: "calle", author: "Carl Rådetorp") {
        capability "Actuator"
        ...
        fingerprint profileId: "0104",
                    ...
                    manufacturer: "Innr",
                    model: "AE 264"
            // Fingerprint for IBBR RB286C (tunable white / color temperature)
        fingerprint profileId: "0104",
                    endpointId: "01",
...
// ---- Helpers ----
private String ctToName(Integer kelvin) {
...
def parse(String description) {
    ...
    // Also parse raw read attributes for StartUpOnOff & CT
    def descMap = zigbee.parseDescriptionAsMap(description)

    // Parse Color Control cluster (0x0300) -> Color Temperature attribute 0x0007 (mireds)
    if (descMap?.cluster in ["0300", "300"]) {
        ...
        if (attr && "0007".equalsIgnoreCase(attr.toString())) {
            try {
                ...
                if (!raw) return
                Integer mired = Integer.parseInt(raw, 16)
                if (mired && mired > 0) {
                    Integer kelvin = Math.round(1000000.0 / mired)
                    sendEvent(name: "colorTemperature", value: kelvin, descriptionText: "Color Temperature set to ${kelvin} K")
                    def cname = ctToName(kelvin as Integer)
                    sendEvent(name: "colorName", value: cname)
                    if (debugLogging) log.debug "Parsed CT ${mired} mireds -> ${kelvin}K (${cname})"
                } else {
                    if (debugLogging) log.warn "Ignoring CT report with mireds=0 (avoiding divide by zero)"
                }
            } catch (e) {
                if (debugLogging) log.warn "Unable to parse ColorTemperature value: ${descMap.value} (${e})"
            }
        }
    }
...
def setColorTemperature(kelvin, level = null, duration = null) {
    ...
    // Kelvin -> mireds (uint16)
    Integer mired = Math.max(1, Math.round(1000000.0 / k) as Integer) // clamp to avoid 0
    ...
    // Ensure on when making a visible change
    ...
    if (level != null && (level as String).toString().isNumber()) {
    ...
    def cname = ctToName(k as Integer)
    sendEvent(name: "colorName", value: cname)
...
def refresh() {
    ...
    return zigbee.onOffRefresh() +
           ...
           zigbee.readAttribute(0x0300, 0x0007)    // ColorTemperature
}
...
// Some platforms/apps expect a healthCheck() command when "Health Check" capability is present
